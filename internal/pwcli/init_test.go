...
func TestParseInitArgs(t *testing.T) {
	for _, testcase := range []struct {
		...
		{name: "oauth is a value of the same question", args: []string{"demo", "--auth=oauth"}, want: initOptions{Name: "demo", Database: true, Engine: engineSQLite, Redis: true, Devbox: true, Auth: authOAuth, Session: sessionRDB, Skills: skillsClaude}},
		{name: "oauth drops a stray emulator flag, having no issuer to emulate", args: []string{"demo", "--auth=oauth", "--devidp"}, want: initOptions{Name: "demo", Database: true, Engine: engineSQLite, Redis: true, Devbox: true, Auth: authOAuth, Session: sessionRDB, Skills: skillsClaude}},
